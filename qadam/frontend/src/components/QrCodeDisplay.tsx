  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-xs">
      {failed ? (
        ...
        <div className="mx-auto flex h-[280px] w-full max-w-[280px] items-center justify-center text-sm text-slate-500">
          Rendering QR...
        </div>
      )}
      {caption && <p className="mt-3 text-center text-xs text-slate-500">{caption}</p>}
      <div className="mt-3 flex items-start gap-2">
        <code className="min-w-0 flex-1 break-all rounded-xl bg-slate-50 px-2.5 py-2 text-xs text-slate-700">
          {data}
        </code>
        <button
          type="button"
          onClick={copyPayload}
          className="qadam-btn-secondary shrink-0 px-2.5 py-2 text-xs"
        >
          {copied ? (
